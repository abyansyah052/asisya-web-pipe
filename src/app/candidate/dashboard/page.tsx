...
export default function CandidateDashboard() {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-slate-50">
            {/* Instructions Modal - Beautiful Design */}
            {showInstructionsModal && selectedExam && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
                    <div className="bg-white sm:rounded-3xl rounded-t-3xl shadow-2xl max-w-2xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden animate-in fade-in zoom-in duration-200">
                        ...
                        <div className="relative bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 px-4 sm:px-8 py-5 sm:py-8 text-white overflow-hidden">
                            {/* Decorative circles */}
                            ...
                            <div className="relative z-10">
                                ...
                                <p className="text-blue-100 text-xs sm:text-sm md:text-base line-clamp-2">{selectedExam.description}</p>
                            </div>
                        </div>
                        ...
                        <div className="px-4 sm:px-8 py-4 sm:py-6 max-h-[50vh] overflow-y-auto">
                            ...
                            <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-4 sm:mb-6">
                                ...
                                <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl sm:rounded-2xl border border-blue-100">
                                    <div className="p-1.5 sm:p-2 bg-blue-500 rounded-lg sm:rounded-xl text-white shrink-0">
                                        <FileText className="w-4 h-4 sm:w-5 sm:h-5" />
                                    </div>
                                    <div className="min-w-0">
                                        <p className="text-[10px] sm:text-xs text-blue-600 font-medium">Jenis</p>
                                        <p className="text-sm sm:text-lg font-bold text-blue-900">Psikotes</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-slate-200">
                                <h4 className="font-bold text-slate-800 mb-3 sm:mb-4 flex items-center gap-2 text-sm sm:text-base">
                                    <BookOpen className="w-4 h-4 sm:w-[18px] sm:h-[18px] text-blue-600" />
                                    Petunjuk Pengerjaan
                                </h4>
                        ...
                        <div className="px-4 sm:px-8 py-4 sm:py-5 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row gap-2 sm:gap-3 sm:justify-between sm:items-center">
                            ...
                            <button
                                ...
                                className="flex items-center justify-center gap-2 px-4 sm:px-8 py-2.5 sm:py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-xl shadow-lg shadow-blue-500/30 transition-all transform hover:scale-[1.02] active:scale-95 order-1 sm:order-2 text-sm sm:text-base"
                            >
            ...
            <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white shadow-sm">
                <div className="px-3 sm:px-4 md:px-8 lg:px-40 flex h-14 sm:h-16 items-center justify-between max-w-[1440px] mx-auto">
                    <div className="flex items-center gap-2 sm:gap-3 text-slate-900 min-w-0">
                        ...
                        <h2 className="text-sm sm:text-lg font-bold tracking-tight truncate">Asisya Ujian Portal</h2>
                    </div>
                    <button
                        onClick={handleLogout}
                        className="flex items-center gap-2 rounded-lg px-3 sm:px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 transition-colors shrink-0"
                    >
                        ...
                        <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="mt-1 sm:mt-2 text-2xl sm:text-4xl font-bold text-blue-600 tracking-tight">{todoExams.length}</p>
                                </div>
                                <div className="flex size-10 sm:size-14 items-center justify-center rounded-full bg-blue-50 text-blue-600">
                        ...
                        <div className="border-b border-slate-200">
                            <div className="flex gap-4 sm:gap-8">
                                <button
                                    ...
                                    className={`relative flex items-center gap-1.5 sm:gap-2 pb-2 sm:pb-3 pt-2 text-xs sm:text-sm font-semibold transition-colors ${activeTab === 'todo' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-700'
                                        }`}
                                >
                                ...
                                <button
                                    ...
                                    className={`relative flex items-center gap-1.5 sm:gap-2 pb-2 sm:pb-3 pt-2 text-xs sm:text-sm font-semibold transition-colors ${activeTab === 'completed' ? 'text-blue-600' : 'text-slate-500 hover:text-slate-700'
                                        }`}
                                >
                                    ...
                                    <div className="flex flex-col gap-4">
                                        {todoExams.length === 0 ? (
                                            ...
                                            todoExams.map((exam) => (
                                                <div
                                                    key={exam.id}
                                                    className="group relative flex flex-col sm:flex-row overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-all hover:shadow-md hover:border-blue-200"
                                                >
                                                    {/* Image placeholder with gradient - Left side on desktop */}
                                                    <div
                                                        className="w-full sm:w-40 md:w-56 h-32 sm:h-auto bg-gradient-to-br from-blue-500 to-blue-700 shrink-0 flex items-center justify-center"
                                                    >
                                                    ...
                                                    <div className="flex flex-1 flex-col justify-between p-4 sm:p-5 md:p-6">
                                                            ...
                                                            <div className="flex justify-between items-start gap-2">
                                                                <div className="space-y-1.5 sm:space-y-2 min-w-0 flex-1">
                                                                    <div className="flex flex-wrap items-center gap-2">
                                                                        <span className="inline-flex items-center rounded-full bg-blue-50 px-2 sm:px-2.5 py-0.5 text-[10px] sm:text-xs font-bold text-blue-700 ring-1 ring-inset ring-blue-700/10">
                                                                            Psikotes
                                                                        </span>
                                                                    ...
                                                                    <h3 className="text-base sm:text-lg md:text-xl font-bold text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-2">
                                                                        {exam.title}
                                                                    </h3>
                                                        ...
                                                        <div className="mt-4 sm:mt-6 flex items-center justify-end border-t border-slate-100 pt-3 sm:pt-4 md:border-0 md:pt-0">
                                                            <button
                                                                ...
                                                                className="flex w-full md:w-auto items-center justify-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white px-4 sm:px-6 py-2.5 sm:py-3 text-sm font-bold shadow-lg shadow-blue-500/25 transition-all active:scale-95"
                                                            >
                                                                Mulai Ujian
            ...
            {/* Footer */}
            <footer className="mt-auto border-t border-slate-200 bg-white py-4 sm:py-6">
                <div className="px-3 sm:px-4 md:px-8 lg:px-40 max-w-[1440px] mx-auto flex flex-col items-center justify-between gap-3 sm:gap-4 text-xs sm:text-sm text-slate-500">
                    <p className="text-center">© 2025 Asisya Consulting. All rights reserved.</p>
                    <div className="flex gap-4 sm:gap-6">
                        ...
                        <a className="hover:text-blue-600 transition-colors" href="#">Syarat</a>
                    </div>
                </div>
            </footer>
        </div>
